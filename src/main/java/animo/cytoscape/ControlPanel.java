...
public class ControlPanel extends JPanel
{
    ...
    public ControlPanel()
    {
        ...
        JButton loadSimulationDataButton = new JButton(new AbstractAction("Load simulation data...")
        {
            ...
            public void actionPerformed(ActionEvent e)
            {
                ...
                if (net != null)
                {
                    ...
                    resultViewer.addToPanel(Animo.getCytoscape().getCytoPanel(Animo.getResultPanelContainer().getCytoPanelName()));
        ...
        new ChangeSecondsAction(Animo.getCytoscapeApp().getCyApplicationManager(), changeSecondsPerPointbutton); //This manages the button for changing the number of seconds per UPPAAL time unit
        ...
        JButton runButton = new JButton(new RunAction(remoteUppaal, serverName, serverPort, smcUppaal, timeTo, nSimulationRuns, computeAvgStdDev, overlayPlot,
                smcFormula));
        ...
        JLabel legend = new JLabel(new ImageIcon(this.getClass().getResource("/animo/resources/legend.png")));
        legend.setPreferredSize(new Dimension(200, 650));
        buttons.add(new LabelledField("Legend", new JScrollPane(legend, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,
                ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED)), new GridBagConstraints(0, yPositionCounter++, 1, 1, 1, 1, GridBagConstraints.CENTER,
        ...
        options.addActionListener(new ActionListener()
        {
            ...
            public void actionPerformed(ActionEvent e)
            {
                final JDialog optionsDialog = new JDialog(Animo.getCytoscape().getJFrame(), "ANIMO Options", Dialog.ModalityType.APPLICATION_MODAL);
                ...
                { //If the property wasn't there, we shouldn't assume that the user knows of its existence: so we make its effects null
                    unc = 0;
                }
                ...
                okButton.addActionListener(new ActionListener()
                {
                    ...
                    public void actionPerformed(ActionEvent ev)
                    {
                        ...
                        if (useUncertainty.isSelected())
                        {
                            ...
                            {
                                JOptionPane.showMessageDialog(Animo.getCytoscape().getJFrame(), "\"" + uncertaintyValue + "\" is not a number.", "Error",
                                        JOptionPane.ERROR_MESSAGE);
                                return;
                            }
                        }
                        ...
                        {
                            JOptionPane.showMessageDialog(Animo.getCytoscape().getJFrame(), "Unexpected problem: " + ex);
                            ex.printStackTrace();
                        }
