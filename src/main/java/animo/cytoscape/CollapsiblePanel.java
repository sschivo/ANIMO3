 ...
 */
//Modified in order to use icons from the resources in ANIMO, otherwise the code is identical
...
public class CollapsiblePanel extends JPanel {
    ...
    //Trying to do the same, but with a checkbox.. And seems to work =)
    public CollapsiblePanel(JCheckBox component) {
        component.addItemListener(new CollapsiblePanel.ExpandAndCollapseAction());
        titleComponent = component;
        collapsed = !component.isSelected();
        commonConstructor();
	...
	private ImageIcon[] createExpandAndCollapseIcon() {
		...
		iconURL = getClass().getResource("/arrow_collapsed.gif"); // Thread.currentThread().getContextClassLoader().getResource("arrow_collapsed.gif");
		if (iconURL != null) {
			iconArrow[COLLAPSED] = new ImageIcon(iconURL);
		}
		iconURL = getClass().getResource("/arrow_expanded.gif"); // Thread.currentThread().getContextClassLoader().getResource("arrow_expanded.gif");
		if (iconURL != null) {
