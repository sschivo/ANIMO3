...
public class EdgeDialog extends JDialog
{
    ...
    private static final String DECIMAL_FORMAT_STRING = "##.####", SAVE = "Save", CANCEL = "Cancel", SCENARIO = Model.Properties.SCENARIO,
            CANONICAL_NAME = Model.Properties.CANONICAL_NAME, INCREMENT = Model.Properties.INCREMENT, UNCERTAINTY = Model.Properties.UNCERTAINTY;
    ...
    public EdgeDialog(final Window owner, final CyNetwork network, final CyEdge edge)
    {
        ...
        //super("Reaction " + Cytoscape.getNodeAttributes().getAttribute(edge.getSource().getSUID(), "canonicalName") + ((Integer.parseInt(Cytoscape.getEdgeAttributes().getAttribute(edge.getSUID(), "increment").toString()) >= 0)?" --> ":" --| ") + Cytoscape.getNodeAttributes().getAttribute(edge.getTarget().getSUID(), "canonicalName"));
        ...
        final JRadioButton positiveIncrement = new JRadioButton("Activation"), negativeIncrement = new JRadioButton("Inhibition");
        ...
        controls.add(new JButton(new AbstractAction(SAVE)
        {
            ...
            public void actionPerformed(ActionEvent e)
            {
                ...
                for (int i = 0; i < paramFields.length; i++)
                {
                    ...
                    else if (paramFields[i] instanceof JLabel)
                    { //Scenario 1/2 reactants

                    }
                    else if (paramFields[i] instanceof Box)
                    { //Scenario 3 reactants
                        Box e1Box = (Box) paramFields[i++], //Two consecutive boxes
                        e2Box = (Box) paramFields[i];
                        JComboBox<String> listE1act = (JComboBox<String>) (e1Box.getComponent(1)), listE1 = (JComboBox<String>) (e1Box.getComponent(2)), listE2act = (JComboBox<String>) (e2Box
                                .getComponent(1)), listE2 = (JComboBox<String>) (e2Box.getComponent(2));
                        ...
                        else
                        {
                            ...
                            Animo.setRowValue(network.getRow(edge), Model.Properties.REACTANT_IS_ACTIVE_INPUT + "E1", Boolean.class, false);
                        }
                        ...
                        else
                        {
                            ...
                            Animo.setRowValue(network.getRow(edge), Model.Properties.REACTANT_IS_ACTIVE_INPUT + "E2", Boolean.class, false);
                        }
                    }
                }
                ...
                Animo.setRowValue(network.getRow(edge), Model.Properties.OUTPUT_REACTANT, Long.class, edge.getTarget().getSUID());
                //edgeAttrib.getRow(edge).set(Model.Properties.DESCRIPTION, description.getText());
                Animo.setRowValue(network.getRow(edge), Model.Properties.DESCRIPTION, String.class, description.getText());
    ...
    private void updateParametersBox(CyEdge edge, Box parametersBox, Scenario selectedScenario, int increment)
    {
        ...
        else if (currentlySelectedScenario == 1)
        {
            ...
            String downstreamReactantName = n.getRow(edge.getTarget()).get(Model.Properties.CANONICAL_NAME, String.class);
            JLabel label1 = new JLabel("E = active " + upstreamReactantName), label2 = new JLabel("S = " + ((increment >= 0) ? "inactive " : "active ")
                    + downstreamReactantName);
        ...
        else if (currentlySelectedScenario == 2)
        {
            ...
            final JComboBox<String> listE1 = new JComboBox<String>(reactantAliases), listE2 = new JComboBox<String>(reactantAliases), listE1act = new JComboBox<String>(
                    activeInactive), listE2act = new JComboBox<String>(activeInactive);
            ...
            listE1.addActionListener(new ActionListener()
            {
                @Override
                public void actionPerformed(ActionEvent e)
                { //Show the selected node in the network, so that the user can be sure of which it is.
