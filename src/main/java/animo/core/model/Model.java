...
public class Model implements Serializable
{
    public static class Properties
    {
        ...
        /**
         * Property belonging to a node. The current activity level of a node. Expressed as a relative number representing INITIAL_LEVEL / NUMBER_OF_LEVELS, so it is a double number in [0, 1]
         */
        ...
        /**
         * This value is multiplied to the time bounds as a counterbalance to any change in seconds per point. This allows us to avoid having to directly modify the parameters of scenarios.
         */
        ...
        /**
         * (prefix for a parameter of a reaction) The Cytoscape ID for a reactant. The actual name of this property is for example _REACTANT_S, _REACTANT_E, _REACTANT_E1, _REACTANT_E2
         */
        public static final String REACTANT_ID = "_REACTANT_";
        /**
         * (prefix for a parameter of a reaction) tells whether the input for a reaction considers the active or inactive part of the given reactant. The actual name of this property is for example _REACTANT_ACT_S, _REACTANT_ACT_E, _REACTANT_ACT_E1, _REACTANT_ACT_E2
         */
    ...
    private static void checkParameters(Integer nMinutesToSimulate) throws AnimoException
    {
        ...
        if (nLvl == null)
        {
            ...
            String inputLevels = JOptionPane.showInputDialog(Animo.getCytoscape().getJFrame(),/*(Component) monitor,*/
                    "Missing number of levels for the network. Please insert the max number of levels", Integer.valueOf(defaultNLevels));
            if (inputLevels != null)
            {
                try
                {
        ...
        if (nSecPerPoint == null)
        {
            ...
            String inputSecs = JOptionPane.showInputDialog(Animo.getCytoscape().getJFrame(),/*(Component) monitor,*/
        ...
        if (secStepFactor == null)
        {
            ...
            Animo.setRowValue(network.getRow(network), Model.Properties.SECS_POINT_SCALE_FACTOR, Double.class, secStepFactor);
        }
        ...
        if (noReactantsPlotted)
        {
            if (JOptionPane.showConfirmDialog(Animo.getCytoscape().getJFrame(),/*(Component) monitor,*/
                    "No graphs will be shown. Do you still want to continue?", "No reactants selected for plotting", JOptionPane.YES_NO_OPTION) == JOptionPane.NO_OPTION)
            {
        ...
        for (CyEdge edge : network.getEdgeList())
        {
            ...
            else
            {
                ...
                //network.getRow(edge).set(Model.Properties.SCENARIO, scenarioIdx);
                Animo.setRowValue(network.getRow(edge), Model.Properties.SCENARIO, Integer.class, scenarioIdx);
                throw new AnimoException("The reaction " + edgeName + " has an invalid scenario setting (" + scenarioIdx
                        + "). Now I set it to the first available: please set the correct parameters.");
            }
            ...
            for (String param : paramNames)
            {
                ...
                if (d == null)
                {
                    ...
                    Animo.setRowValue(network.getRow(edge), param, Double.class, scenario.getDefaultParameterValue(param));
                }
            }
            ...
            if (scenarioIdx == 2)
            {
                if (network.getRow(edge).isSet(Model.Properties.REACTANT_ID + "E1"))
                {
                    ...
                    Animo.setRowValue(network.getRow(edge), Model.Properties.REACTANT_ID + "E1", String.class, edge.getSource().getSUID().toString());
                }
                if (network.getRow(edge).isSet(Model.Properties.REACTANT_IS_ACTIVE_INPUT + "E1"))
                {
                    ...
                    Animo.setRowValue(network.getRow(edge), Model.Properties.REACTANT_IS_ACTIVE_INPUT + "E1", Boolean.class, true);
                }
                if (network.getRow(edge).isSet(Model.Properties.REACTANT_ID + "E2"))
                {
                    ...
                    Animo.setRowValue(network.getRow(edge), Model.Properties.REACTANT_ID + "E2", String.class, edge.getTarget().getSUID().toString());
                }
                if (network.getRow(edge).isSet(Model.Properties.REACTANT_IS_ACTIVE_INPUT + "E2"))
                {
                    ...
                    Animo.setRowValue(network.getRow(edge), Model.Properties.REACTANT_IS_ACTIVE_INPUT + "E2", Boolean.class, true);
                }
                ...
                {
                    nameBuilder.append(network.getRow(edge.getTarget()).get(Model.Properties.CANONICAL_NAME, String.class));
                }
                else
                {
            ...
            case 1:
                ...
                if (!source.get(Model.Properties.ENABLED, Boolean.class) && target.get(Model.Properties.ENABLED, Boolean.class))
                {
                    throw new AnimoException("Please check that reactant \"" + source.get(Model.Properties.CANONICAL_NAME, String.class)
                            + "\" is enabled, or reaction \"" + edgeName + "\" cannot stay enabled.");
                }
                else if (!source.get(Model.Properties.ENABLED, Boolean.class) && !target.get(Model.Properties.ENABLED, Boolean.class))
                {
                    throw new AnimoException("Please check that both reactants \"" + source.get(Model.Properties.CANONICAL_NAME, String.class) + "\" and \""
                            + target.get(Model.Properties.CANONICAL_NAME, String.class) + "\" are enabled, or reaction \"" + edgeName
                            + "\" cannot stay enabled.");
                }
                else if (source.get(Model.Properties.ENABLED, Boolean.class) && !target.get(Model.Properties.ENABLED, Boolean.class))
                {
                    throw new AnimoException("Please check that reactant \"" + target.get(Model.Properties.CANONICAL_NAME, String.class)
                            + "\" is enabled, or reaction \"" + edgeName + "\" cannot stay enabled.");
                }
                else
                {
            ...
            case 2:
                ...
                {
                    throw new AnimoException("Please check that reactant \"" + e1.get(Model.Properties.CANONICAL_NAME, String.class)
                            + "\" is enabled, or reaction \"" + edgeName + "\" cannot stay enabled.");
                }
                else if (!e1.get(Model.Properties.ENABLED, Boolean.class) && !e2.get(Model.Properties.ENABLED, Boolean.class)
                        && network.getRow(edge.getTarget()).get(Model.Properties.ENABLED, Boolean.class))
                {
                    throw new AnimoException("Please check that both reactants \"" + e1.get(Model.Properties.CANONICAL_NAME, String.class) + "\" and \""
                            + e2.get(Model.Properties.CANONICAL_NAME, String.class) + "\" are enabled, or reaction \"" + edgeName + "\" cannot stay enabled.");
                }
                else if (e1.get(Model.Properties.ENABLED, Boolean.class) && !e2.get(Model.Properties.ENABLED, Boolean.class)
                        && network.getRow(edge.getTarget()).get(Model.Properties.ENABLED, Boolean.class))
                {
                    throw new AnimoException("Please check that reactant \"" + e2.get(Model.Properties.CANONICAL_NAME, String.class)
                            + "\" is enabled, or reaction \"" + edgeName + "\" cannot stay enabled.");
                }
                else if (!e1.get(Model.Properties.ENABLED, Boolean.class) && e2.get(Model.Properties.ENABLED, Boolean.class)
                        && !network.getRow(edge.getTarget()).get(Model.Properties.ENABLED, Boolean.class))
                {
                    throw new AnimoException("Please check that both reactants \"" + e1.get(Model.Properties.CANONICAL_NAME, String.class) + "\" and \""
                            + network.getRow(edge.getTarget()).get(Model.Properties.CANONICAL_NAME, String.class) + "\" are enabled, or reaction \"" + edgeName
                            + "\" cannot stay enabled.");
                }
                else if (!e1.get(Model.Properties.ENABLED, Boolean.class) && !e2.get(Model.Properties.ENABLED, Boolean.class)
                        && !network.getRow(edge.getTarget()).get(Model.Properties.ENABLED, Boolean.class))
                {
                    throw new AnimoException("Please check that reactants \"" + e1.get(Model.Properties.CANONICAL_NAME, String.class) + "\", \""
                        ...
                        && !network.getRow(edge.getTarget()).get(Model.Properties.ENABLED, Boolean.class))
                {
                    throw new AnimoException("Please check that both reactants \"" + e2.get(Model.Properties.CANONICAL_NAME, String.class) + "\" and \""
                            + network.getRow(edge.getTarget()).get(Model.Properties.CANONICAL_NAME, String.class) + "\" are enabled, or reaction \"" + edgeName
                            + "\" cannot stay enabled.");
                }
                else
            ...
            if (tempScaleUpstream != null)
            {
                ...
                Animo.setRowValue(network.getRow(edge.getSource()), Model.Properties.LEVELS_SCALE_FACTOR, Double.class, scaleUpstream);
                // network.getRow(edge.getTarget()).set(Model.Properties.LEVELS_SCALE_FACTOR, scaleUpstream);
                Animo.setRowValue(network.getRow(edge.getTarget()), Model.Properties.LEVELS_SCALE_FACTOR, Double.class, scaleUpstream);
        ...
        double maxSecStep = Double.POSITIVE_INFINITY, //The lower bound of the "valid" interval for secs/step (minSecStep) is the maximum of the lower bounds we find for it, while the upper bound (maxSecStep) is the minimum of all upper bounds. This is why we compute them in this apparently strange way
        ...
        while (edges.hasNext())
        {
            ...
            double scaleFactorR1 = network.getRow(edge.getSource()).get(Model.Properties.NUMBER_OF_LEVELS, Integer.class).doubleValue();
            double scaleFactorR2 = network.getRow(edge.getTarget()).get(Model.Properties.NUMBER_OF_LEVELS, Integer.class).doubleValue();
            CyRow e1 = network.getRow(edge.getSource());
            CyRow e2 = network.getRow(edge.getTarget());
        ...
        if (nMinutesToSimulate != null && nMinutesToSimulate >= 0)
        { //If we were requested to perform a simulation (and not simply translate the model "out of curiosity" or for model checking), we were given also a time limit for the simulation. We need to check whether that time limit is a "large number"
        ...
        if (!Double.isInfinite(minSecStep) && secPerStep < minSecStep)
        { //Notice that this check is made last because it is the most important: if we set seconds/point to a value less than the computed minimum, the time values will be so large that UPPAAL will not be able to understand them, thus producing no result
    ...
    public static Model generateModelFromCurrentNetwork(TaskMonitor monitor, Integer nMinutesToSimulate, boolean generateTables) throws AnimoException
    {
        ...
        {
            model.getProperties().let("deltaAlternating").be(network.getRow(network).get("deltaAlternating", Boolean.class));
        }
        if (network.getRow(network).isSet("useOldResetting"))
        {
            model.getProperties().let("useOldResetting").be(network.getRow(network).get("useOldResetting", Boolean.class));
        }
        model.getProperties().let(Model.Properties.NUMBER_OF_LEVELS).be(network.getRow(network).get(Model.Properties.NUMBER_OF_LEVELS, Integer.class));
        model.getProperties().let(Model.Properties.SECONDS_PER_POINT).be(network.getRow(network).get(Model.Properties.SECONDS_PER_POINT, Double.class));
        double secStepFactor = network.getRow(network).get(Model.Properties.SECS_POINT_SCALE_FACTOR, Double.class);
        ...
        for (int i = 0; nodes.hasNext(); i++)
        {
            ...
            r.let(Model.Properties.NUMBER_OF_LEVELS).be(network.getRow(node).get(Model.Properties.NUMBER_OF_LEVELS, Integer.class));
            r.let(Model.Properties.LEVELS_SCALE_FACTOR).be(network.getRow(node).get(Model.Properties.LEVELS_SCALE_FACTOR, Double.class));
            ...
            r.let(Model.Properties.INITIAL_LEVEL).be(network.getRow(node).get(Model.Properties.INITIAL_LEVEL, Integer.class));
        ...
        for (int i = 0; edges.hasNext(); i++)
        {
            ...
            if (scenarioIdx >= 0 && scenarioIdx < scenarios.length)
            {
                ...
                {
                case 0:
                    levelsScaleFactor = 1.0 / model.getReactant(reactant).get(Model.Properties.NUMBER_OF_LEVELS).as(Integer.class)
                            * model.getReactant(catalyst).get(Model.Properties.NUMBER_OF_LEVELS).as(Integer.class);
                    break;
                case 1:
                    levelsScaleFactor = 1.0 * model.getReactant(catalyst).get(Model.Properties.NUMBER_OF_LEVELS).as(Integer.class);
                    break;
                case 2:
            ...
            r.let(Model.Properties.SCENARIO_PARAMETER_K).be(network.getRow(edge).get(Model.Properties.SCENARIO_PARAMETER_K, Double.class));
            ...
            if (scenarioIdx == 2)
            { //actually, they are both catalysts
                String cata, reac;
                ...
                //levelsScaleFactor /= 2*nodeAttributes.getDoubleAttribute(network.getRow(edge).get(Model.Properties.REACTANT_ID + "E2",String.class)), Model.Properties.LEVELS_SCALE_FACTOR);
            }
            else
            {
            ...
            if (scenarioIdx == 2)
            {
                reactionAlias += " AND " + model.getReactant(r.get(Model.Properties.REACTANT).as(String.class)).getName();
            }
            ...
            { //No tabels were requested, so find only min and max time to be used in the computation of reaction activityRatio
    ...
    public void setReactants(Map<String, Reactant> reactants)
    { //The next 7 methods are to keep compatibility with java beans and XML encoder/decoder
