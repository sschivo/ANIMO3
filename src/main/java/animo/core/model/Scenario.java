...
/**
 * Represents a scenario for a reaction in the model. There are 3 predefined scenarios, each one with its set of parameters.
 */
public abstract class Scenario {
	...
	public Double computeFormula(int r1Level, int nLevelsR1, boolean activeR1, int r2Level, int nLevelsR2,
			boolean activeR2) {
		double rate = computeRate(r1Level, nLevelsR1, activeR1, r2Level, nLevelsR2, activeR2);
		if (rate > 1e-8) {
			// return Math.max(0, (int)Math.round(1 / rate)); //We need to put at least 1 because otherwise the reaction will keep happening forever (it is not very nice not to let
			// time pass..)
			return 1.0 / rate;
		} else {
	...
	public abstract double computeRate(int r1Level, int nLevelsR1, boolean activeR1, int r2Level, int nLevelsR2,
			boolean activeR2);

	/**
	 * Generate the times table based on the scenario formula and parameters.
	 * 
	 * @param nLevelsReactant1
	 *            The total number of levels of reactant1 (the enzyme or catalyst) + 1 (!! Note the +1! Basically, this is the number of columns of the time table, so when using it
	 *            as the number of activity levels of the enzyme we must subtract 1)
	 * @param activeR1
	 *            true if the R1 input to the formula is the concentration of active reactant 1
	 * @param reactant1IsDownstream
	 *            true when the reactant 1 is a downstream reactant (the reaction time depends on its activity, but we treat limit cases as their nearest neighbours to ensure that
	 *            boolean networks behave correctly)
	 * @param nLevelsReactant2
	 *            The total number of levels of reactant2 (the substrate) + 1 (!! Note the +1)
	 * @param activeR2
	 *            true if the R2 input to the formula is the concentration of active reactant 2
	 * @param reactant2IsDownstream
	 *            see reactant1IsDownstream
	 * @return Output is a list for no particular reason anymore. It used to be set as a Cytoscape property, but these tables can become clumsy to be stored inside the model,
	 *         slowing down the loading/saving processes in the Cytoscape interface. We now compute the tables on the fly and output them directly as reference constants in the
	 *         UPPPAAL models, saving also memory.
	 */
	public List<Double> generateTimes(int nLevelsReactant1, boolean activeR1, boolean reactant1IsDownstream,
			int nLevelsReactant2, boolean activeR2, boolean reactant2IsDownstream) {
		...
		for (; i < limitI; i++) {
			int j, limitJ;
			if (!activeR1) { // We depend on the inactivity of R1, which when j = 0 is completely inactive (first column --> R1 = 0), so the first column will have the smallest
								// values
				j = 0;
			...
			if (!activeR1) { // We depend on the inactivity of R1, which in the last column is completely active. So the last column has all infinite
				if (reactant1IsDownstream) {
					times.add(computeFormula(nLevelsReactant1 - 2, nLevelsReactant1 - 1, activeR1, i,
							nLevelsReactant2 - 1, activeR2));
				} else {
		...
		if (!activeR2) { // We depend on the inactivity of R2, which in the last row is completely active. So the last row has all infinite
			for (int j = 0; j < nLevelsReactant1; j++) {
				if (reactant2IsDownstream) {
					times.add(computeFormula(j, nLevelsReactant1 - 1, activeR1, nLevelsReactant2 - 2,
							nLevelsReactant2 - 1, activeR2));
				} else {
	 ...
	 * @param reactantName
	 * @param linkedTo
	 *            contains <Cytoscape reactant ID, whether we consider the active or inactive part of that reactant as parameter for this scenario>
	 */
