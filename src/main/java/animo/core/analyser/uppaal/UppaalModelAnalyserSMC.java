...
/**
 * This class is currently used for all queries. Computes the requested analysis on the given ANIMO model, translating it into the corresponding UPPAAL model depending on which
 * query is asked. Always uses the model produced from VariablesModelSMC, which is tailored to work with UPPAAL SMC engine. As the model does not use priorities, we employ it also
...
public class UppaalModelAnalyserSMC implements ModelAnalyser<LevelResult> {
	...
	public class VariablesInterpreterConcrete {
		...
		/**
		 * Parse the UPPAAL output containing a trace run on the given model until the given time The trace is the result of one SMC simulation query in the form simulate 1
		 * [<=timeTo] { var1, var2, ..., varn }
		 * 
		 * @param m
		 *            The model on which the trace is based
		 * @param output
		 *            The stream from which to read the trace
		 * @param timeTo
		 *            The time up to which the simulation trace arrives (or should arrive)
		 * @return The SimpleLevelResult containing a series for each of the reactants in the model, showing the activity levels of that reactant for each time point of the trace.
		 */
		public SimpleLevelResult analyse(Model m, InputStream output, int timeTo) throws IOException {
			long startTime = System.currentTimeMillis();

			BufferedReader br = new BufferedReader(new InputStreamReader(output));
			String line = br.readLine();

			while (line != null && !line.startsWith(" -- Property is satisfied."))
				line = br.readLine();

			long endTime = System.currentTimeMillis();
			System.err.println(" took " + AnimoActionTask.timeDifferenceFormat(startTime, endTime));

			return analyseFromStream(m, br, timeTo);
		}

		private SimpleLevelResult analyseFromStream(Model m, BufferedReader br, int timeTo)
			...
			for (Reactant r : m.getReactantCollection()) {
				...
				if (nLvl == null) {
					...
					String inputLevels = JOptionPane.showInputDialog("Missing number of levels for reactant \"" + name
							+ "\" (" + r.getId() + ").\nPlease insert the max number of levels for \"" + name + "\"",
							maxNumberOfLevels);
					if (inputLevels != null) {
						try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (m.getReactant(reactantId) != null) {
					...
					if (reaction != null) {
						levels.put(reaction.get(Model.Properties.CYTOSCAPE_ID).as(String.class),
				...
				while (pointMatcher.find()) {
					...
					if (m.getReactant(reactantId) != null) { // it is a reactant, so rescale the value as activity level, using the maximum number of levels
						if (numberOfLevels.get(reactantId) != maxNumberOfLevels) {
							level = level / numberOfLevels.get(reactantId) * maxNumberOfLevels;
						}
					} else { // It is not a reactant: for the moment, this can only mean that it is a reaction duration
						reactionId = reactantId.substring(0, reactantId.lastIndexOf(dot)); // It is in the form R6_R4.T, so we remove the ".T" and keep the rest, which is used as
																							// reaction ID in the Model object
						reaction = m.getReaction(reactionId);
						if (reaction != null) {
							...
							if (level == 0 || level == VariablesModel.INFINITE_TIME
									|| minTime == VariablesModel.INFINITE_TIME) { // I put also level == 0 because otherwise we go in the "else" and we divide by 0 =)
								level = 0;
							} else if (activityIntervalWidth > -2) { // If there are not orders of magnitude of difference between minimum and maximum, we can simply use a normal
																		// linear scale as we did before
								level = 1.0 * minTime / level;
							} else {
								// level = 1.0 * minTime / level;
								level = (activityIntervalWidth - Math.log10(1.0 * minTime / level))
										/ activityIntervalWidth;
							}
						}
					}
					...
					if (rMap.isEmpty() || timeTo == -1 || (rMap.get(rMap.lastKey()) != level && time <= timeTo)) { // if we didn't register a variation, we don't plot a point
																													// Also, if the current time is already over the requested
																													// simulation time, we don't put it in the data (but still read
																													// the values, to clear the stream)
						rMap.put(time, level);
					}
				}
				// System.err.println();
			...
			// if (time < timeTo) { //if the state of the system remains unchanged from a certain time on (and so UPPAAL terminates on that point), but we asked for a later time,
			// we add a final point where all data remain unchanged, so that the user can see the "evolution" up to the requested point
			// we do it always, because there can be some situations in which reactants are not read while time increases, and thus we can reach the end of time without having an
			// updated value for each reactant
			if (timeTo != -1) {
		...
		/**
		 * Parse the UPPAAL output containing a trace run on the given model until the given time The trace is the (counter)example resulting from a (true) <> query or (false) []
		 * query.
		 * 
		 * @param m
		 *            The model on which the trace is based
		 ...
		 * @throws IOException
		 */
		public SimpleLevelResult analyseNormalTrace(Model m, InputStream output, int timeTo) throws IOException {
			BufferedReader br = new BufferedReader(new InputStreamReader(output));
			// String line = null;
		...
		private SimpleLevelResult analyseNormalTraceFromStream(Model m, BufferedReader br, int timeTo)
				throws IOException {
			// rand = new Random(randInitializer.nextLong());
			lastComputed = -1; // Lascialo a -1! Quando chiedo di calcolare il random dentro un intervallo, se lastComputed e' < 0 uso come lastComputed il lowerBound
								// dell'intervallo.

			Map<String, SortedMap<Double, Double>> levels = new HashMap<String, SortedMap<Double, Double>>();

			String line = null, lastLine = null;
			Pattern globalTimeLowerBoundPattern = Pattern
					.compile("[' ']globalTime>[=]?[-+]?[0-9]*\\.?[0-9]+([eE][-+]?[0-9]+)?"); // Pattern.compile("globalTime[=][0-9]+"); //The new pattern supports also numbers like
																								// 3.434252e+06, which can occur(!!)
			Pattern globalTimeUpperBoundPattern = Pattern
					.compile("[' ']globalTime<[=]?[-+]?[0-9]*\\.?[0-9]+([eE][-+]?[0-9]+)?");
			...
			int timeLB = -1, timeUB = -1, lastTimeLB = -1, lastTimeUB = -1; // This is done so that when parsing the lines we correctly set the initial value for every variable
																			// (also the .T's), because we will read the latest value of those variables ALSO for time = 0
			...
			while ((line = br.readLine()) != null) {
				...
				while (stateMatcher.find()) {
					...
					if (reactantId.equals("globalTime")
							|| reactantId.equals("r")
							|| reactantId.startsWith("reactant")
							|| reactantId.startsWith("output") // private variables are not taken into account
							|| reactantId.equals("r1") || reactantId.equals("r2") || reactantId.endsWith(dot + "r")
							...
							|| reactantId.endsWith(dot + "deltaAlternating"))
						continue;
					if (reactantId.endsWith(dot + "T")) {
						reactantId = m.getReaction(reactantId.substring(0, reactantId.lastIndexOf(dot + "T")))
								.get(Model.Properties.CYTOSCAPE_ID).as(String.class);
					}
					...
					if (m.getReactant(reactantId) != null) { // it is a reactant, so rescale the value as activity level, using the maximum number of levels
						...
						if (nLvl == null) {
							...
							String inputLevels = JOptionPane.showInputDialog("Missing number of levels for reactant \""
									+ name + "\" (" + r.getId() + ").\nPlease insert the max number of levels for \""
			...
			double currentTime = 0; // we use it to see where we are along the simulation run (we get its value from the parseLine function), and thus compute a % of advancement
									// with respect to timeTo
			int lastPercent = 0;
			while ((line = br.readLine()) != null) {
				if (!line.startsWith("State"))
					continue;
				...
				line = " " + br.readLine(); // If I don't add a space at the start of the line, the patterns will not match correctly. I need a space in front of the "globalTime"
											// string because otherwise I would also match things like clockOfWhichIDontReallyCare-globalTime<=12345
				...
				if (!timeMatcherPrecise.find()) {
					...
					Matcher timeMatcherLB = globalTimeLowerBoundPattern.matcher(line), timeMatcherUB = globalTimeUpperBoundPattern
							.matcher(line);
					...
					if (timeMatcherLB.find()) {
						...
						if (valueLB.substring(0, 1).equals("=")) {
							if (valueLB.substring(1, 2).equals("-")) {
								newTimeLB = (int) Math
										.round(Double.parseDouble(valueLB.substring(2, valueLB.length())));
							} else {
								newTimeLB = (int) Math
										.round(Double.parseDouble(valueLB.substring(1, valueLB.length())));
							}
						} else {
							if (valueLB.substring(0, 1).equals("-")) {
								newTimeLB = (int) (Math
										.round(Double.parseDouble(valueLB.substring(1, valueLB.length())))); // why +1??
							} else {
								newTimeLB = (int) (Math
										.round(Double.parseDouble(valueLB.substring(0, valueLB.length())))); // why +1??
							}
						}
					...
					if (timeMatcherUB.find()) {
						...
						if (valueUB.substring(0, 1).equals("=")) {
							if (valueUB.substring(1, 2).equals("-")) {
								newTimeUB = (int) Math
										.round(Double.parseDouble(valueUB.substring(2, valueUB.length())));
							} else {
								newTimeUB = (int) Math
										.round(Double.parseDouble(valueUB.substring(1, valueUB.length())));
							}
						} else {
							if (valueUB.substring(0, 1).equals("-")) {
								newTimeUB = (int) (Math
										.round(Double.parseDouble(valueUB.substring(1, valueUB.length())))); // why +1??
							} else {
								newTimeUB = (int) (Math
										.round(Double.parseDouble(valueUB.substring(0, valueUB.length())))); // why +1??
							}
						}
					...
					if (newTimeLB > timeLB || newTimeUB > timeUB) {
						...
						currentTime = parseLine(m, timeLB, timeUB, timeTo, lastLine, numberOfLevels, maxNumberOfLevels,
								statePattern, levels);
						timeLB = newTimeLB;
						timeUB = newTimeUB;
						lastLine = line;
					...
					if (lastTimeLB == timeLB) {
						...
						if (newTimeLB == lastTimeLB) {
							...
							timeLB = lastTimeLB;
							timeUB = lastTimeUB;
							currentTime = parseLine(m, timeLB, timeUB, timeTo, lastLine, numberOfLevels,
									maxNumberOfLevels, statePattern, levels);
						}
					}
				}
			...
			if (lastState != null) {
				...
				if (!timeMatcherPrecise.find()) {
					...
					currentTime = parseLine(m, newTimeLB, newTimeUB, timeTo, lastState, numberOfLevels,
							maxNumberOfLevels, statePattern, levels);
				} else {
					String value = (timeMatcherPrecise.group().split("=")[1]);
					int newTimeLB = -1;
			...
			if (timeTo != -1) {
				...
				for (String reactantName : levels.keySet()) {
					SortedMap<Double, Double> values = levels.get(reactantName);
					double lastValue = values.get(values.lastKey());
					values.put(lastPoint, lastValue);
				}
			}
			// }
		 ...
		 * 
		 * @param m
		 *            The model on which the result is based
		 * @param smcOutput
		 *            The stream from which to read the UPPAAL output
		 * @return The parsed SMCResult containing the boolean/numerical query answer
		 */
		public SMCResult analyseSMC(Model m, InputStream smcOutput, long startTime) throws AnalysisException {
			...
			try {
				while ((line = br.readLine()) != null) {
					...
					System.err.println(" took "
							+ AnimoActionTask.timeDifferenceFormat(startTime, System.currentTimeMillis()));
					...
					if (line == null) { // Property is just true or false
						...
						if (line.startsWith("Showing example trace") || line.startsWith("Showing counter example")) { // has a trace result
							if (monitor != null) {
								monitor.setStatusMessage("Property is " + (boolResult ? "true" : "false")
										+ ". Parsing " + (boolResult ? "" : "counter") + "example trace.");
							}
							...
							String lowerBoundS = line.substring(line.indexOf('[') + 1, line.lastIndexOf(",")), upperBoundS = line
							...
							try {
								...
								br.close();
								throw new AnalysisException("Unable to understand probability bounds for the result \""
										+ line + "\"");
							}
				...
				try {
					throw new AnalysisException("Unable to understand UPPAAL SMC output: " + readTheRest(line, br), ex);
				} catch (IOException e) {
					throw new AnalysisException("Unable to understand UPPAAL SMC output");
				}
			}
			...
			try {
				throw new AnalysisException("Unable to understand UPPAAL SMC output: " + readTheRest(line, br));
			} catch (IOException e) {
				throw new AnalysisException("Unable to understand UPPAAL SMC output");
			}
		}
		 ...
		 * 
		 * @param currentLine
		 *            The line from which to start looking for a confidence value
		 * @param br
		 *            The reader from which to continue reading UPPAAL output
		 * @return The confidence value
		 */
		private double findConfidence(String currentLine, BufferedReader br) throws IOException {
			if (currentLine == null)
				return 1.0;
			double confidence = 0;
			...
			if (weHaveAProblem) {
				...
				if (errorMsg.length() > 200) {
					errorMsg = errorMsg.substring(0, 100) + endLine + " [...] " + endLine
							+ errorMsg.substring(errorMsg.length() - 100);
				}
				JOptionPane.showMessageDialog(Animo.getCytoscape().getJFrame(), errorMsg, "UPPAAL SMC Exception",
		...
		private double parseLine(Model m, double timeLB, double timeUB, int timeTo, String line,
				Map<String, Double> numberOfLevels, int maxNumberOfLevels, Pattern statePattern,
			...
			while (stateMatcher.find()) {
				...
				/*
				 * if (reactantId.equals("c") || reactantId.equals("globalTime") || reactantId.equals("r") || reactantId.startsWith("input_reactant_") ||
				 * reactantId.startsWith("output_reactant_") || reactantId.equals("r1") || reactantId.equals("r2") || maximumValues.get(reactantId) == null) continue; //we check
				 * whether it is a private variable
				 */
				if (!reactantId.endsWith(dot + "T") && !levels.containsKey(reactantId))
					continue;
				...
				if (m.getReactant(reactantId) != null) { // it is a reactant, so rescale the value as activity level, using the maximum number of levels
					...
					reactionId = reactantId.substring(0, reactantId.lastIndexOf(dot + "T")); // It is in the form R6_R4.T, so we remove the ".T" and keep the rest, which is used as
																								// reaction ID in the Model object
					reaction = m.getReaction(reactionId);
					if (reaction != null) {
						chosenMap = reaction.get(Model.Properties.CYTOSCAPE_ID).as(String.class);
				...
				if (rMap.isEmpty() || (rMap.get(rMap.lastKey()) != level)) { // if we didn't register a variation, we don't plot a point
					/*
					 * if (rMap.lastKey() < time - 1) { //We use this piece to explicitly keep a level constant when it is not varying (i.e., the graph will never contain
					 * non-vertical,non-horizontal lines) rMap.put((double)(time - 1), rMap.get(rMap.lastKey())); }
					 */
		...
		/**
		 * Used when reporting an error about the SMC query answer. As UPPAAL SMC is still in beta stage, we do our best to understand its output, but if we fail we try at least to
		 * speed up the process of changing the parsing according to a possibly new output format.
		 * 
		 * @param line
		 *            The entire line which caused the problem
		 * @param br
		 *            The buffered reader from which to continue to read the rest of the input
		 * @return A string containing the rest of the input
		 * @throws IOException
		 */
		private String readTheRest(String line, BufferedReader br) throws IOException {
	...
	public static double TIME_SCALE = 0.2; // the factor by which time values are mutiplied before being output on the .csv file (it answers the question
											// "how many real-life minutes does a time unit of the model represent?")
	...
	/**
	 * Perform a simple simulation run on the given model, up to the given time. Please notice that we use the VariablesModelSMC class to transform the model into the UPPAAL input,
	 * because that type of model allows us to get concrete simulation traces, while the other model (obtained by using VariableModel) can only produce symbolic traces. Notice
	 * furthermore that we do not analyse the compiled model via the tracer tool, because (for an unexplained reason) the only way to obtain exact time values for the globalTime
	 * clock along the simulation is reading the direct verifyta output. If we try to parse its file output with the tracer tool, all exact values are substituted by a series of
	 * clock difference inequalities, which need to be solved to regain the knowledge which was there in the first place (!!). So, for simplicity's sake, we simply analyse the
	 * direct output, even if it is in principle a little slower than analysing the file output (it would certainly be if the file output also contained exact clock values instead
	 * of bounds).
	 * 
	 * @param m
	 *            The model to analyse
	 * @param timeTo
	 *            the length of the simulation, in UPPAAL time units. The translation from real-life minutes to UPPAAL time units is made in RunAction.performNormalAnalysis.
	 * @return The SimpleLevelResult showing as series the activity levels of all reactants present in the model during the simulation period
	 */
	@Override
	public SimpleLevelResult analyze(final Model m, final int timeTo) throws AnalysisException {
		...
		try {
			...
			String nomeFileModello = modelFile.getAbsolutePath(), nomeFileQuery = queryFile.getAbsolutePath();
			...
			new Thread() {
				...
				public void run() {
					try {
						 ...
						 * proc.getOutputStream().close(); proc.getErrorStream().close(); }
						 */
						resultVector.add(new UppaalModelAnalyserSMC.VariablesInterpreterConcrete(monitor).analyse(m,
								inputStream, timeTo));
					} catch (IOException e) {
			...
			if (actionTask != null) {
				...
				try {
					...
					proc.destroy();
					throw new InterruptedException("Interrupted (1)");
				}
				while (resultVector.isEmpty()) { // if the verifyta process is completed, we may still need to wait for the analysis thread to complete
					Thread.sleep(100);
				}
			...
			if (proc.exitValue() != 0 && ((result = resultVector.firstElement()) == null || result.isEmpty())) {
				...
				throw new AnalysisException(errorBuilder.toString());
			} else {
				result = resultVector.firstElement();
			}
	...
	/**
	 * Returns the SMCResult that we obtain from analysing with UPPAAL the given model with the given probabilistic query.
	 * 
	 * @param m
	 *            The model to analyse
	 * @param probabilisticQuery
	 *            The probabilistic query already translated with correct UPPAAL time units and reactant names (ex. Pr[<=12000](<> reactant0 > 40), and not Pr[<=240](<> MK2 > 40),
	 *            which is instead what the user inserts. The translation is made in RunAction.performSMCAnalysis)
	...
	public SMCResult analyzeSMC(final Model m, String probabilisticQuery) throws AnalysisException {
		...
		try {
			...
			String nomeFileModello = modelFile.getAbsolutePath(), nomeFileQuery = queryFile.getAbsolutePath(), nomeFileOutput = nomeFileModello
					.substring(0, nomeFileModello.lastIndexOf(".")) + ".output";
			File fileOutput = new File(nomeFileOutput);
			fileOutput.deleteOnExit();

			// the following string is used in order to make sure that the name of .xtr output files is unique even when we are called by an application which is multi-threaded
			// itself (it supposes of course that the input file is unique =))
			String[] cmd = new String[3];
			...
			final Vector<SMCResult> resultVector = new Vector<SMCResult>(1); // this has no other reason than to hack around the fact that an internal class needs to have all
																				// variables it uses declared as final
			...
			new Thread() {
				...
				public void run() {
					try {
						SMCResult result = new UppaalModelAnalyserSMC.VariablesInterpreterConcrete(monitor).analyseSMC(
								m, inputStream, startTime);
			...
			if (proc.exitValue() != 0
					&& ((result = resultVector.firstElement()) == null || (result.getResultType() == SMCResult.RESULT_TYPE_TRACE && result
							.getLevelResult().isEmpty()))) {
				StringBuilder errorBuilder = new StringBuilder();
				errorBuilder.append("[" + nomeFileModello + "] Verify result: " + proc.exitValue() + "\n");
				if (result == null) {
					errorBuilder.append(" null result\n");
