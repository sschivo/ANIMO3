...
//				outString = new StringWriter();
				...
				template.append("double rate" + r.getId() + "() {\n");
...
//				StringBuilder template = new StringBuilder("<template><name>Reactant_" + r.getId() + "</name><parameter>int&amp; R, const int MAX</parameter><declaration>int[-1, 1] delta" + (dAlternating?", deltaNew = 0, deltaOld = 0, deltaOldOld = 0, deltaOldOldOld = 0;\nbool deltaAlternating = false":"") + ";\ntime_t tL, tU;\nclock c;\ndouble_t totalRate;\n\n\n" + (dAlternating?"void updateDeltaOld() {\n\tdeltaOldOldOld = deltaOldOld;\n\tdeltaOldOld = deltaOld;\n\tdeltaOld = deltaNew;\n\tdeltaNew = delta;\n\tdeltaAlternating = false;\n\tif (deltaOldOldOld != 0) { //We have updated delta at least 4 times, so we can see whether we have an oscillation\n\t\tif (deltaNew == deltaOldOld &amp;&amp; deltaOld == deltaOldOldOld &amp;&amp; deltaNew != deltaOld) { //Pairwise equal and alternating (e.g. +1, -1, +1, -1): we are oscillating\n\t\t\tdeltaAlternating = true;\n\t\t\tdeltaNew = deltaOld = deltaOldOld = deltaOldOldOld = 0;\n\t\t}\n\t}\n}\n\n":"") + "void update() {\n");
				...
				for (Reaction re : influencingReactions) {
					...
					switch (scenario) {
						case 0:
							template.append("\tdouble " + re.getId() + "_r = k_" + re.getId() + " * " + m.getReactant(re.get(Model.Properties.CATALYST).as(String.class)).getId() + ";\n");
							break;
						case 1: case 2:
							template.append("\tdouble " + re.getId() + "_r = k_" + re.getId() + " * ");
							if (activeR2) {
								template.append(m.getReactant(re.get(Model.Properties.REACTANT).as(String.class)).getId());
							} else {
								template.append("(" + m.getReactant(re.get(Model.Properties.REACTANT).as(String.class)).getId() + "Levels - " + m.getReactant(re.get(Model.Properties.REACTANT).as(String.class)).getId() + ")");
							}
							template.append(" * ");
							if (activeR1) {
								template.append(m.getReactant(re.get(Model.Properties.CATALYST).as(String.class)).getId() + ";\n");
							} else {
								template.append("(" + m.getReactant(re.get(Model.Properties.CATALYST).as(String.class)).getId() + "Levels - " + m.getReactant(re.get(Model.Properties.CATALYST).as(String.class)).getId() + ");\n");
							}
							break;
						default:
							break;
...
//				}
				...
				if (influencingReactions.size() == 1) {
					...
					template.append(((increment > 0)?"":"-") + re.getId() + "_r");
//					if (uncertainty != 0) {
...
//					}
				...
				template.append("\t} else if (" + r.getId() + " + r &lt; 0) {\n");
				template.append("\t\treturn -" + r.getId() + ";\n");
				template.append("\t}\n");
...
//				out.append(newLine);
			...
			for (Reactant r : m.getSortedReactantList()) {
				...
				template.append(r.getId() + " = " + r.getId() + "Init");
			}
			template.append("</label></transition>");
