...
public class UppaalInvoker {
	...
	/**
	 * The tracer is used to combine a compiled model and an xtr file to generate a human-readable trace.
	 */
	 ...
	 * @param tracerPath
	 *            the path to the tracer executable
	 * @param verifytaPath
	 *            the path to the verifyta executable
	 * @param leaveFiles
	 *            should we leave the temporary files
	 */
	...
	/**
	 * Produces a trace of the single query in the file. This method should only be used for a single query that results a trace.
	 * 
	 * @param model
	 *            the UPPAAL model to use
	 * @param query
	 *            the query to use
	 * @return a trace that satisfies the query, or {@code null} if the query could not be satisfied
	 * @throws IOException
	 *             if the trace could not be generated for some reason
