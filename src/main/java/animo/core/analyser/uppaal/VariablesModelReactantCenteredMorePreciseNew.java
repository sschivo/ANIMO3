...
public class VariablesModelReactantCenteredMorePreciseNew extends VariablesModel {
	...
	protected void appendTemplates(StringBuilder out, Model m) {
		try {
			...
			for (Reactant r : m.getSortedReactantList()) {
				...
				template.append("time_t tL = INFINITE_TIME,\n\t   tU = INFINITE_TIME,\n\t   tHalf = INFINITE_TIME,\n\t   tHalfOld = INFINITE_TIME;\n");
				template.append("clock c;\n");
				template.append("double_t totalRate = INFINITE_TIME_DOUBLE,\n\t\t oldRate = INFINITE_TIME_DOUBLE;\n");
				...
				template.append("\ttHalfOld = tHalf;\n");
				...
				template.append("bool direction_changed() {\n\treturn delta != oldDelta;\n}\n\n");
				template.append("bool direction_unchanged() {\n\treturn delta == oldDelta;\n}\n\n");
				...
				template.append("<location id=\"id1\" x=\"-1528\" y=\"-728\"><committed/></location>"); //Decide if c > tU and do c = tU in case
				template.append("<location id=\"id2\" x=\"-1664\" y=\"-612\"><committed/></location>"); //Decide if c >= tHalfOld and react in case
				template.append("<location id=\"id3\" x=\"-1844\" y=\"-586\"><committed/></location>"); //Decide if direction is changed
				...
				template.append("<transition><source ref=\"id1\"/><target ref=\"id9\"/><label kind=\"guard\" x=\"-1598\" y=\"-705\">c &gt; tU</label><label kind=\"assignment\" x=\"-1598\" y=\"-688\">c = tU</label><nail x=\"-1528\" y=\"-688\"/><nail x=\"-1608\" y=\"-688\"/></transition>");
				template.append("<transition><source ref=\"id0\"/><target ref=\"id3\"/><label kind=\"guard\" x=\"-1955\" y=\"-680\">c &lt; tU</label><label kind=\"assignment\" x=\"-1955\" y=\"-663\">update()</label><nail x=\"-1895\" y=\"-586\"/></transition>");
				template.append("<transition><source ref=\"id0\"/><target ref=\"id9\"/><label kind=\"guard\" x=\"-1818\" y=\"-696\">c &gt;= tU</label><nail x=\"-1838\" y=\"-680\"/><nail x=\"-1742\" y=\"-680\"/></transition>");
				template.append("<transition><source ref=\"id6\"/><target ref=\"id8\"/><label kind=\"assignment\" x=\"-1344\" y=\"-728\">update()</label></transition>");
				template.append("<transition><source ref=\"id3\"/><target ref=\"id2\"/><label kind=\"guard\" x=\"-1819\" y=\"-629\">direction_changed()</label><nail x=\"-1827\" y=\"-612\"/></transition>");
				template.append("<transition><source ref=\"id3\"/><target ref=\"id8\"/><label kind=\"guard\" x=\"-1819\" y=\"-586\">direction_unchanged()</label><nail x=\"-1470\" y=\"-586\"/></transition>");
				template.append("<transition><source ref=\"id2\"/><target ref=\"id9\"/><label kind=\"guard\" x=\"-1700\" y=\"-688\">c &gt;= tHalfOld</label><label kind=\"assignment\" x=\"-1700\" y=\"-671\">tL = 0, tU = 0,\nc = 0, delta = oldDelta</label></transition>");
				template.append("<transition><source ref=\"id2\"/><target ref=\"id8\"/><label kind=\"guard\" x=\"-1598\" y=\"-629\">c &lt; tHalfOld</label><label kind=\"assignment\" x=\"-1598\" y=\"-612\">c = 0</label><nail x=\"-1479\" y=\"-612\"/></transition>");
				int y1 = -904,
					y2 = -888,
					y3 = -848,
					incrY = -40;
