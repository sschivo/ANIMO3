...
public class VariablesModelReactantCenteredMorePreciseNew4 extends VariablesModel {
		...
		protected void appendTemplates(StringBuilder out, Model m) {
			try {
				...
				for (Reactant r : m.getSortedReactantList()) {
					...
					template.append("<location id=\"id0\" x=\"-1896\" y=\"-728\"><name x=\"-1960\" y=\"-752\">stubborn</name><label kind=\"invariant\" x=\"-1960\" y=\"-740\">c2 &lt;= 1</label></location>");
					...
					for (Reaction re : influencingReactions) { //Transitions from waiting to stubborn
						...
						switch (scenario) {
							case 0:
								if (catalyst.get(REACTANT_INDEX).as(Integer.class) != r.get(REACTANT_INDEX).as(Integer.class) && !alreadyOutputReactants.contains(catalyst)) {
									alreadyOutputReactants.add(catalyst);
									template.append("<transition><source ref=\"id9\"/><target ref=\"id0\"/><label kind=\"synchronisation\" x=\"-1832\" y=\"" + y1 + "\">reacting[" + m.getReactant(re.get(CATALYST).as(String.class)).get(REACTANT_INDEX).as(Integer.class) + "]?</label><label kind=\"guard\" x=\"-1752\" y=\"" + (y2-16) + "\">c &lt; tL</label><label kind=\"assignment\" x=\"-1832\" y=\"" + (y1+16) + "\">c2 = 0</label><nail x=\"-1752\" y=\"" + y2 + "\"/><nail x=\"-1840\" y=\"" + y2 + "\"/></transition>");
									y1 += incrY;
									y2 += incrY;
								}
								break;
							...
							case 2: //In this case, CATALYST = E1, REACTANT = E2 (the two upstream reactants)
								...
								if (reactant.get(REACTANT_INDEX).as(Integer.class) != r.get(REACTANT_INDEX).as(Integer.class) && !alreadyOutputReactants.contains(reactant)) {
									alreadyOutputReactants.add(reactant);
									template.append("<transition><source ref=\"id9\"/><target ref=\"id0\"/><label kind=\"synchronisation\" x=\"-1832\" y=\"" + y1 + "\">reacting[" + m.getReactant(re.get(REACTANT).as(String.class)).get(REACTANT_INDEX).as(Integer.class) + "]?</label><label kind=\"guard\" x=\"-1752\" y=\"" + (y2-16) + "\">c &lt; tL</label><label kind=\"assignment\" x=\"-1832\" y=\"" + (y1+16) + "\">c2 = 0</label><nail x=\"-1752\" y=\"" + y2 + "\"/><nail x=\"-1840\" y=\"" + y2 + "\"/></transition>");
									y1 += incrY;
									y2 += incrY;
								}
								break;
