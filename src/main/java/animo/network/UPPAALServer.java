...
/**
 * The remote server. Implements the methods for simulation run analysis and SMC analysis. Listens for remote connections on the given port.
 */
public class UPPAALServer extends UnicastRemoteObject implements iUPPAALServer {
	...
	public static void main(String[] args) throws AnimoException {
		try {
			int rmiPort = DEFAULT_RMI_PORT, // TODO: These two ports MUST be opened in the firewall
			servicePort = DEFAULT_SERVICE_PORT;
			if (args.length < 2) {
				System.err.println("No ports given: defaults are " + rmiPort + " for RMI registry, and " + servicePort
				...
				System.err.println("Using " + rmiPort + " port for RMI registry, and " + servicePort
						+ " for the actual server");
			}
			System.err
			...
			System.out.println(DATE_FORMAT.format(new Date(System.currentTimeMillis()))
					+ " Server started and listening on port " + rmiPort + ".");
	...
	public SimpleLevelResult analyze(Model m, int timeTo, int nSimulationRuns, boolean computeAvgStdDev,
			boolean overlayPlot) throws IOException, AnalysisException {
		System.out.println(DATE_FORMAT.format(new Date(System.currentTimeMillis()))
				+ " Analyzing \"normal\" model with simulation up to " + timeTo);
	...
	public SMCResult analyze(Model m, String smcQuery) throws AnalysisException, IOException {
		System.out.println(DATE_FORMAT.format(new Date(System.currentTimeMillis()))
				+ " Analyzing \"SMC\" model with query " + smcQuery);
		SMCResult result = new UppaalModelAnalyserSMC/* FasterSymbolicConcretized */(null, null).analyzeSMC(m, smcQuery); // new UppaalModelAnalyserFasterConcrete(null,
																															// null).analyzeSMC(m, smcQuery);
		System.out.println(DATE_FORMAT.format(new Date(System.currentTimeMillis())) + " Done.");
		System.out.println();
		return result;
	}
