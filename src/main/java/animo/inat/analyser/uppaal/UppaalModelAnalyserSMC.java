package animo.inat.analyser.uppaal;
...
import animo.inat.analyser.ModelAnalyser;
...
public class UppaalModelAnalyserSMC implements ModelAnalyser<LevelResult>
{
    ...
    public class VariablesInterpreterConcrete
    {
        ...
        public SimpleLevelResult analyse(Model m, InputStream output, int timeTo) throws IOException
        {
            ...
            System.err.println(" took " + InatActionTask.timeDifferenceFormat(startTime, endTime));

            return analyseFromStream(m, br, timeTo);
        }


        private SimpleLevelResult analyseFromStream(Model m, BufferedReader br, int timeTo) throws NumberFormatException, IOException
        {
            ...
            while ((line = br.readLine()) != null)
            {
                ...
                while (pointMatcher.find())
                {
                    ...
                    if (m.getReactant(reactantId) != null)
                    { //it is a reactant, so rescale the value as activity level, using the maximum number of levels
                    ...
                    { //It is not a reactant: for the moment, this can only mean that it is a reaction duration
                        ...
                        if (reaction != null)
                        {
                            ...
                            if (level == 0 || level == VariablesModel.INFINITE_TIME || minTime == VariablesModel.INFINITE_TIME)
                            { //I put also level == 0 because otherwise we go in the "else" and we divide by 0 =)
                                level = 0;
                            }
                            else if (activityIntervalWidth > -2)
                            { //If there are not orders of magnitude of difference between minimum and maximum, we can simply use a normal linear scale as we did before
                    ...
                    if (rMap.isEmpty() || timeTo == -1 || (rMap.get(rMap.lastKey()) != level && time <= timeTo))
                    { //if we didn't register a variation, we don't plot a point
            ...
            System.err.println("\tParsing the result produced by UPPAAL took " + InatActionTask.timeDifferenceFormat(startTime, endTime));
        ...
        private SimpleLevelResult analyseNormalTraceFromStream(Model m, BufferedReader br, int timeTo) throws IOException
        {
            ...
            while ((line = br.readLine()) != null)
            {
                ...
                while (stateMatcher.find())
                {
                    ...
                    if (reactantId.equals("globalTime")
                            ...
                            || reactantId.equals("r1") || reactantId.equals("r2") || reactantId.endsWith(dot + "r") || reactantId.endsWith(dot + "reactant")
                            || reactantId.endsWith(dot + "output") || reactantId.endsWith(dot + "r1") || reactantId.endsWith(dot + "r2")
                            || reactantId.endsWith(dot + "e") || reactantId.endsWith(dot + "b") || reactantId.endsWith(dot + "c")
                            || reactantId.endsWith(dot + "delta") || reactantId.endsWith(dot + "tL") || reactantId.endsWith(dot + "tU")
                            || reactantId.endsWith(dot + "deltaNew") || reactantId.endsWith(dot + "deltaOld") || reactantId.endsWith(dot + "deltaOldOld")
                            || reactantId.endsWith(dot + "deltaOldOldOld") || reactantId.endsWith(dot + "deltaAlternating"))
                        continue;
                    if (reactantId.endsWith(dot + "T"))
                    {
                        reactantId = m.getReaction(reactantId.substring(0, reactantId.lastIndexOf(dot + "T"))).get(Model.Properties.CYTOSCAPE_ID)
                                .as(String.class);
                    }
            ...
            while ((line = br.readLine()) != null)
            {
                ...
                if (!timeMatcherPrecise.find())
                {
                    ...
                    Matcher timeMatcherLB = globalTimeLowerBoundPattern.matcher(line), timeMatcherUB = globalTimeUpperBoundPattern.matcher(line);
        ...
        public SMCResult analyseSMC(Model m, InputStream smcOutput, long startTime) throws AnalysisException
        {
            ...
            try
            {
                while ((line = br.readLine()) != null)
                {
                    ...
                    System.err.println(" took " + InatActionTask.timeDifferenceFormat(startTime, System.currentTimeMillis()));
                    ...
                    line = br.readLine();
                    if (line == null)
                    { //Property is just true or false
                        res = new SMCResult(boolResult, findConfidence(line, br));
                        br.close();
                        return res;
                    }
                    else
                    { //There are other components to the answer
                        if (line.startsWith("Showing example trace") || line.startsWith("Showing counter example"))
                        { //has a trace result
                            if (monitor != null)
                            {
                                monitor.setStatusMessage("Property is " + (boolResult ? "true" : "false") + ". Parsing " + (boolResult ? "" : "counter")
                                        + "example trace.");
                            }
                        ...
                        else if ((line.indexOf("runs) H") != -1) || (line.indexOf("runs) Pr(..)/Pr(..)") != -1))
                        { //it has boolean result with confidence
                            res = new SMCResult(boolResult, findConfidence(line, br));
                            br.close();
                            return res;
                        }
                        else
                        { //the result is between lower and upper bound
                            String lowerBoundS = line.substring(line.indexOf('[') + 1, line.lastIndexOf(",")), upperBoundS = line.substring(
                            ...
                            {
                                br.close();
                                throw new AnalysisException("Unable to understand probability bounds for the result \"" + line + "\"");
                            }
        ...
        private double findConfidence(String currentLine, BufferedReader br) throws IOException
        {
            ...
            if (!line.contains(objective))
            {
                while ((line = br.readLine()) != null)
                {
                    ...
                    { //This actually means that UPPAAL has found a problem in our model. I do my best to report the error to the user.
                        weHaveAProblem = true;
                    }
                }
            }
        ...
        private double parseLine(Model m, double timeLB, double timeUB, int timeTo, String line, Map<String, Double> numberOfLevels, int maxNumberOfLevels,
                Pattern statePattern, Map<String, SortedMap<Double, Double>> levels)
        {
            Matcher stateMatcher = statePattern.matcher(line);
            String s = null;
            if (timeLB == -1)
            ...
            while (stateMatcher.find())
            {
                ...
                if (rMap.isEmpty() || (rMap.get(rMap.lastKey()) != level))
                { //if we didn't register a variation, we don't plot a point
    ...
    public SimpleLevelResult analyze(final Model m, final int timeTo) throws AnalysisException
    {
        ...
        try
        {
            ...
            if (actionTask != null)
            {
                taskStatus = 0;
                new Thread()
                { //wait for the process to end correctly
                    @Override
                    public void run()
                    {
                        try
                ...
                new Thread()
                { //wait for the process to end by user cancellation
                    @Override
                    public void run()
                    {
                        while (taskStatus == 0)
                ...
                if (errors.isEmpty())
                {
                    ...
                    { //if the verifyta process is completed, we may still need to wait for the analysis thread to complete
                        Thread.sleep(100);
                    }
                }
            }
    ...
    public SMCResult analyzeSMC(final Model m, String probabilisticQuery) throws AnalysisException
    {
        ...
        try
        {
            ...
            String nomeFileModello = modelFile.getAbsolutePath(), nomeFileQuery = queryFile.getAbsolutePath(), nomeFileOutput = nomeFileModello.substring(0,
                    nomeFileModello.lastIndexOf(".")) + ".output";
            ...
            if (proc.exitValue() != 0
                    && ((result = resultVector.firstElement()) == null || (result.getResultType() == SMCResult.RESULT_TYPE_TRACE && result.getLevelResult()
                            .isEmpty())))
            {
                StringBuilder errorBuilder = new StringBuilder();
                errorBuilder.append("[" + nomeFileModello + "] Verify result: " + proc.exitValue() + "\n");
