...
public class SimpleLevelResult extends LevelResult implements Serializable
{
    ...
    { //These 3 are for compatibility with java beans and xml encoding/decoding
    ...
    @Override
    public LevelResult difference(LevelResult subtractFrom, Map<String, String> myMapModelIDtoCytoscapeID, Map<String, String> hisMapCytoscapeIDtoModelID)
    {
            ...
            Map<String, String> hisMapCytoscapeIDtoModel)
    {
        ...
        for (String myKey : levels.keySet())
        {
            ...
            {
                //Skip the nodes the other does not have (the check before this one may have been about a disabled node, which is not present in the uppaal model. So subtractFrom.levels may not contain any value for hisKey)
                continue;
            }
            ...
            {
                //Take first one an then the other as a reference for the X values. Please note: this means that the resulting series will have a number of points that is about as big as the sum of the two input series, so it would not be ideal to continue to compute differences of differences of differences...
                if (k <= minDuration)
                {
                    mRes.put(
                            k,
                            maxNLevels
                                    * (subtractFrom.getInterpolatedConcentration(hisKey, k) / subtractFrom.getNumberOfLevels() - this
                                            .getInterpolatedConcentration(myKey, k) / this.getNumberOfLevels()));
                }
            }
