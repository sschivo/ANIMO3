...
public class HeatChart
{
    ...
    /**
     * During the port this function has been broken, due to the deprecation of most methods used.
     *  A good aternative will have to be found.
     * @param value
     * @return
     */
    ...
    private Color getCellColor(double value)
    {
        ...
        /** for (ObjectMapping om : mappings)
         {
             if (om instanceof ContinuousMapping<Double, Double>)
             {
                 ContinuousMapping<Double, Double> m = (ContinuousMapping<Double, Double>) om;
                 int nPunti = m.getPointCount();
                 int idx = 0;
                 double minVal = Double.POSITIVE_INFINITY, maxVal = Double.NEGATIVE_INFINITY, range = 0;
                 for (ContinuousMappingPoint<Double, Double> p : m.getAllPoints())
                 {
                 ...
                 for (; idx < nPunti; idx++)
                 {
                     ContinuousMappingPoint<Double, Double> punto = m.getPoint(idx);
                 ...
                 if (idx == nPunti)
                 {
                     ...
                     return m.getPoint(nPunti - 1).getRange().equalValue;
                 }
                 ContinuousMappingPoint<Double, Double> p1 = m.getPoint(idx1), p2 = m.getPoint(idx2);
                 minVal = p1.getValue();
                 maxVal = p2.getValue();
                 range = maxVal - minVal;
