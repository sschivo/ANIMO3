...
import animo.inat.network.UPPAALClient;
...
public class RunAction extends InatActionTask
{
    private class RunTask extends AbstractTask
    {
        ...
        public RunTask() throws InatException
        {
            ...
            catch (NumberFormatException ex)
            {
                ...
                {
                    throw new InatException("Unable to understand the number of minutes requested for the simulation.");
                }
            }
         ...
         */
        private void performNormalAnalysis(TaskMonitor monitor) throws NumberFormatException, NotBoundException, AnalysisException, IOException
        {
            ...
            if (remoteUppaal.isSelected())
            {
                ...
                monitor.setStatusMessage("Forwarding the request to the server " + serverName.getText() + ":" + serverPort.getText());
            ...
            else
            {
                SwingUtilities.invokeLater(new Runnable()
                {
                    ...
                    public void run()
                    {
                        ...
                        final InatResultPanel resultViewer = new InatResultPanel(model, result, scale, app.getCurrentNetwork());
         ...
         * @throws AnalysisException 
         */
        private void performSMCAnalysis(TaskMonitor monitor) throws NumberFormatException, NotBoundException, AnalysisException, IOException
        {
            ...
            SwingUtilities.invokeLater(new Runnable()
            {
                ...
                public void run()
                {
                    JOptionPane.showMessageDialog(Animo.getCytoscape().getJFrame(), result.toString(), "Result", JOptionPane.INFORMATION_MESSAGE);
                }
            });
        ...
        @Override
        public void run(TaskMonitor monitor) throws InatException, NumberFormatException, NotBoundException, IOException
        {
            needToStop = false;
            ...
            if (noReactantsPlotted && !smcUppaal.isSelected())
            {
                EventQueue.invokeLater(new Runnable()
                {
                    ...
                    public void run()
                    {
                        JOptionPane.showMessageDialog(Animo.getCytoscape().getJFrame(),/*(Component) monitor,*/
                                "No reactants selected for plot: select at least one reactant to be plotted in the graph.", "Error", JOptionPane.ERROR_MESSAGE);
                    }
                });
     ...
     */
    public RunAction(/* TODO: moet hier iets mee? InatPlugin plugin,*/JCheckBox remoteUppaal, JTextField serverName, JTextField serverPort,
            JRadioButton smcUppaal, JFormattedTextField timeToFormula, JFormattedTextField nSimulationRuns, JRadioButton computeAvgStdDev,
            JRadioButton overlayPlot, JTextField smcFormula)
    {
