...
import animo.inat.util.HeatChart;
...
public class Graph extends JPanel implements MouseListener, MouseMotionListener, MouseWheelListener, ActionListener, ComponentListener
{
    ...
    public static final String AUTOGRAPH_WINDOW_TITLE = "AutoGraph", //If we are a window, this will be our (lame) title
            OPEN_LABEL = "Add data from CSV...", SAVE_LABEL = "Save as PNG...",
            RENDER_TO_JPG_LABEL = "Render to JPG...",
            EXPORT_VISIBLE_LABEL = "Export visible as CSV...", CLEAR_LABEL = "Clear Data",
            INTERVAL_LABEL = "Graph interval...",
            ZOOM_RECTANGLE_LABEL = "Zoom rectangle", ZOOM_EXTENTS_LABEL = "Zoom extents", CLOSE_LABEL = "Close",
            SHOW_SIZE_LABEL = "Show picture size",
            SHOW_ZOOM_LEVEL_LABEL = "Show zoom level", SHOW_THIN_AXES = "Show thin axes",
            SHOW_THICK_AXES = "Show thick axes",
            SET_Y_LABEL_LABEL = "Set Y label", STEP_SHAPED_GRAPH_LABEL = "Step-shaped graph",
            HEATMAP_GRAPH_LABEL = "Heat-map graph",
            CSV_FILE_EXTENSION = ".csv", CSV_FILE_DESCRIPTION = "CSV file", DEFAULT_CSV_FILE = "/local/schivos/Data_0-240_TNF100.csv", //"/local/schivos/aData1_0-1440_normalized_MK2_JNK1_IKK_with_stddev.csv", //"/local/schivos/aData1_0-1440_times5_normalized_better_onlyMK2_JNK1_IKK_con_stddev.csv",
    ...
    public void drawAxes(Graphics2D g, Rectangle bounds, Rectangle pictureBounds)
    {
        ...
        { //If we can't fit the text, we don't try to put it anywhere
    ...
    public void drawLegend(Graphics2D g, Rectangle bounds)
    {
        ...
        for (Series series : data)
        {
            ...
            if (!series.drawSymbol(g, bounds.x + 15 * SCALA, bounds.y + 10 * SCALA + nLegend * 20 * SCALA, 10 * SCALA, 10 * SCALA))
            { //If the series has no symbol, then we draw a line
                g.drawLine(bounds.x + 5 * SCALA, bounds.y + 10 * SCALA + nLegend * 20 * SCALA, bounds.x + 25 * SCALA, bounds.y + 10 * SCALA + nLegend * 20
                        * SCALA);
            }
    ...
    public void exportVisible(String fileName) throws FileNotFoundException, IOException
    {
        ...
        { //we use only the enabled series
    ...
    {
        if (x >= 5 * SCALA && x <= legendBounds.width - 5 * SCALA && y >= 5 * SCALA && y <= legendBounds.height - 5 * SCALA)
        {
    ...
    public void mouseClicked(MouseEvent e)
    {
        if (e.getClickCount() == 1)
        {
            ...
            { //Center button
                ...
                { //you can disable/enable series only if the legend is visible
    ...
    public void mouseReleased(MouseEvent e)
    {
        ...
        else if (drawingZoomRectangle)
        {
            ...
            maximumX = Math.max(zoomRectangleBounds.x, e.getX()), minimumY = Math.min(zoomRectangleBounds.y, e.getY()), maximumY = Math.max(
                    zoomRectangleBounds.y, e.getY());
            if (zoomExtentsBounds == null)
            {
            ...
            this.setDrawArea((int) (((minimumX - 1 - BORDER_X * SCALA) / scale.getXScale() + scale.getMinX())), (int) ((maximumX - 1 - BORDER_X * SCALA)
                    / scale.getXScale() + scale.getMinX()),
                    (int) ((this.getBounds().height + 1 - maximumY - BORDER_Y * SCALA) / scale.getYScale() + scale.getMinY()), (int) ((this.getBounds().height
                            + 1 - minimumY - BORDER_Y * SCALA)
                            / scale.getYScale() + scale.getMinY()));
            zoomRectangleBounds = null;
            drawingZoomRectangle = false;
            this.setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
            needRedraw = true;
    ...
    public void paint(Graphics g1, int areaWidth, int areaHeight)
    {
        ...
        if (useHeatMap)
        { //TODO: will not work when we consider overlay graphs (will show nothing at all).
            if (heatMapImg == null || needRedraw)
            {
                ...
                for (Series s : data)
                {
                    ...
                    {
                        if (points[0].x <= x)
                        { //We make sure that the x we consider is inside the interval where this series has values
            ...
            if (xRedLine > 0)
            { //Esattamente i minuti in tempo umano, ovvero il numero che leggo nell'asse x
                g.setPaint(Color.BLACK);
                Stroke olStroke = g.getStroke();
                g.setStroke(new BasicStroke(3 * SCALA));
                Rectangle bounds = heatMapBounds;
                double scaleLine = bounds.width / this.scale.getMaxX();
                g.drawLine((int) Math.round(bounds.x + xRedLine * scaleLine), bounds.height + bounds.y, (int) Math.round(bounds.x + xRedLine * scaleLine),
                        bounds.y - 10 * SCALA);
        ...
        if (legendBounds == null || !customLegendPosition || this.getWidth() != areaWidth || this.getHeight() != areaHeight)
        {
            ...
            legendBounds = new Rectangle(bounds.width - 1 * SCALA - (int) maxLabelLength, bounds.y + 1 * SCALA, 35 * SCALA + (int) maxLabelLength, 20 * SCALA
                    * nGraphs);
        }
        if (showLegend)
        {
    ...
    public void parseCSV(String fileName, List<String> selectedColumns) throws FileNotFoundException, IOException
    {
        ...
        { //We are in case 2
            ...
            for (int i = 0; i < graphNames.length; i++)
            {
                ...
                {
                    for (P p : grafico)
                    { //before adding the graph data, we update it by rescaling the y values
                        p.y *= scaleFactor;
                    }
                    addSeries(grafico, graphNames[i]);
                }
     ...
     */
    public void parseLevelResult(LevelResult result, Map<String, String> seriesNameMapping, double xScale, List<String> selectedColumns)
    {
        boolean mustRescaleYValues = false; //if we find a column whose name is equal to my constant MAX_Y_STRING, two things can happen:
                                            //1. maxYValue == null, then we update its value with the (only) value present in this special column
                                            //2. maxYValue != null, then we rescale the y values of all the series we find in this csv file to the value of maxYValue,
