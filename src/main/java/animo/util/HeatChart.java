...
/**
 * The <code>HeatChart</code> class describes a chart which can display 3-dimensions of values - x,y and z, where x and y are the usual 2-dimensional axis and z is portrayed by
 * colour intensity. Heat charts are sometimes known as heat maps.
 * 
 * <p>
 ...
 * <h3>Instantiation</h3>
 * <p>
 * Construction of a new <code>HeatChart</code> instance is through its one constructor which takes a 2-dimensional array of <tt>doubles</tt> which should contain the z-values for
 * the chart. Consider this array to be the grid of values which will instead be represented as colours in the chart.
 * 
 * <p>
 * Setting of the x-values and y-values which are displayed along the appropriate axis is optional, and by default will simply display the values 0 to n-1, where n is the number of
 * rows or columns. Otherwise, the x/y axis values can be set with the <code>setXValues</code> and <code>setYValues
 * </code> methods. Both methods are overridden with two forms:
 * 
 * <h4>Object axis values</h4>
 * 
 * <p>
 * The simplist way to set the axis values is to use the methods which take an array of Object[]. This array must have the same length as the number of columns for setXValues and
 * same as the number of rows for setYValues. The string representation of the objects will then be used as the axis values.
 * 
 * <h4>Offset and Interval</h4>
 * 
 * <p>
 * This is convenient way of defining numerical values along the axis. One of the two methods takes an interval and an offset for either the x or y axis. These parameters supply
 * the necessary information to describe the values based upon the z-value indexes. The quantity of x-values and y-values is already known from the lengths of the z-values array
 * dimensions. Then the offset parameters indicate what the first value will be, with the intervals providing the increment from one column or row to the next.
 * 
 * <p>
 ...
 * 
 * <p>
 * In this example, the z-values range from 0.7 to 1.6. The x-values range from the xOffset value 1.0 to 4.0, which is calculated as the number of x-values multiplied by the
 * xInterval, shifted by the xOffset of 1.0. The y-values are calculated in the same way to give a range of values from 0.0 to 6.0.
 * 
 * <h3>Configuration</h3>
 * <p>
 * This step is optional. By default the heat chart will be generated without a title or labels on the axis, and the colouring of the heat map will be in grayscale. A large range
 * of configuration options are available to customise the chart. All customisations are available through simple accessor methods. See the javadoc of each of the methods for more
 * information.
 * 
 * <h3>Output</h3>
 * <p>
 ...
 * <ul>
 * <li><strong>getChartImage()</strong> - The chart will be returned as a <code>BufferedImage</code> object that can be used in any number of ways, most notably it can be inserted
 * into a Swing component, for use in a GUI application.</li>
 * <li><strong>saveToFile(File)</strong> - The chart will be saved to the file system at the file location specified as a parameter. The image format that the image will be saved
 * in is derived from the extension of the file name.</li>
 * </ul>
 * 
 * <strong>Note:</strong> The chart image will not actually be created until either saveToFile(File) or getChartImage() are called, and will be regenerated on each successive call.
 */
public class HeatChart {
	...
	private double cellSizeWidth, cellSizeHeight;
	...
	private void drawHeatMap(Graphics2D chartGraphics, double[][] data) {
		...
		for (int x = 0; x < data.length; x++) {
			for (int y = 0; y < data[x].length; y++) {
				...
				heatMapGraphics.fillRect(cellY, cellX, (int) Math.round(cellSizeWidth) + 2,
						(int) Math.round(cellSizeHeight) + 2);
			}
		}
	...
	private void drawXValues(Graphics2D chartGraphics) {
		...
		for (int i = 0; i < xValues.length; i++) {
			...
			if (xValuesHorizontal) {
				...
				int valueXPos = (int) Math.round(heatMapTL.x + (i * cellSizeWidth)
						+ ((cellSizeWidth / 2) + (xAxisValuesHeight / 2)));
	...
	private void drawYValues(Graphics2D chartGraphics) {
		...
		for (int i = 0; i < yValues.length; i++) {
			...
			if (yValuesHorizontal) {
				...
				int valueYPos = (int) Math.round(heatMapTL.y + (i * cellSizeHeight) + (cellSizeHeight / 2)
						+ (yAxisValuesAscent / 2));
	...
	/**
	 * Returns the font which describes the visual style of the axis values. The axis values are those values displayed alongside the axis bars at regular intervals. Both axis use
	 * the same font.
	 * 
	...
	/**
	 * Generates and returns a new chart <code>Image</code> configured according to this object's currently held settings. By default the image is generated with no transparency.
	 * 
	 * <p>
	 * No chart will be generated until this or the related <code>saveToFile(File)</code> method are called. All successive calls will result in the generation of a new chart
	 * image, no caching is used.
	 * 
	...
	/**
	 * Generates and returns a new chart <code>Image</code> configured according to this object's currently held settings. The given parameter determines whether transparency
	 * should be enabled for the generated image.
	 * 
	 * <p>
	 * No chart will be generated until this or the related <code>saveToFile(File)</code> method are called. All successive calls will result in the generation of a new chart
	 * image, no caching is used.
	...
	/*
	 * Returns how many colour shifts are required from the lowValueColour to get to the correct colour position. The result will be different depending on the colour scale used:
	...
	/**
	 * Returns the scale that is currently in use to map z-value to colour. A value of 1.0 will give a <strong>linear</strong> scale, which will spread the distribution of colours
	 * evenly amoungst the full range of represented z-values. A value of greater than 1.0 will give an <strong>exponential</strong> scale that will produce greater emphasis for
	 * the separation between higher values and a value between 0.0 and 1.0 will provide a <strong>logarithmic</strong> scale, with greater separation of low values.
	 * 
	...
	/**
	 * Returns the frequency of the values displayed along the x-axis. The frequency is how many columns in the x-dimension have their value displayed. A frequency of 2 would mean
	 * every other column has a value shown and a frequency of 3 would mean every third column would be given a value.
	 * 
	...
	/**
	 * Returns the x-values which are currently set to display along the x-axis. The array that is returned is either that which was explicitly set with
	 * <code>setXValues(Object[])</code> or that was generated from the offset and interval that were given to <code>setXValues(double, double)</code>, in which case the object
	 * type of each element will be <code>Double</code>.
	 * 
	...
	/**
	 * Returns the frequency of the values displayed along the y-axis. The frequency is how many rows in the y-dimension have their value displayed. A frequency of 2 would mean
	 * every other row has a value and a frequency of 3 would mean every third row would be given a value.
	 * 
	...
	/**
	 * Returns the y-values which are currently set to display along the y-axis. The array that is returned is either that which was explicitly set with
	 * <code>setYValues(Object[])</code> or that was generated from the offset and interval that were given to <code>setYValues(double, double)</code>, in which case the object
	 * type of each element will be <code>Double</code>.
	 * 
	...
	/**
	 * Returns the 2-dimensional array of z-values currently in use. Each element is a double array which represents one row of the heat map, or all the z-values for one y-value.
	 * 
	 * @return an array of the z-values in current use, that is, those values which will define the colour of each cell in the resultant heat map.
	 */
	...
	/**
	 * Generates a new chart <code>Image</code> based upon the currently held settings and then attempts to save that image to disk, to the location provided as a File parameter.
	 * The image type of the saved file will equal the extension of the filename provided, so it is essential that a suitable extension be included on the file name.
	 * 
	 * <p>
	 * All supported <code>ImageIO</code> file types are supported, including PNG, JPG and GIF.
	 * 
	 * <p>
	 * No chart will be generated until this or the related <code>getChartImage()</code> method are called. All successive calls will result in the generation of a new chart image,
	 * no caching is used.
	 * 
	 * @param outputFile
	 *            the file location that the generated image file should be written to. The File must have a suitable filename, with an extension of a valid image format (as
	 *            supported by <code>ImageIO</code>).
	 * @throws IOException
	 *             if the output file's filename has no extension or if there the file is unable to written to. Reasons for this include a non-existant file location (check with
	 *             the File exists() method on the parent directory), or the permissions of the write location may be incorrect.
	 */
	...
	/**
	 * Sets the colour to be used for the axis values as they will be painted along the axis bars. Both axis use the same colour.
	 * 
	 * <p>
	 * Defaults to Color.BLACK.
	 * 
	...
	/**
	 * Sets the font which describes the visual style of the axis values. The axis values are those values displayed alongside the axis bars at regular intervals. Both axis use the
	 * same font.
	 * 
	 * <p>
	...
	/**
	 * Sets the colour to be used on the background of the chart. A transparent background can be set by setting a background colour with an alpha value. The transparency will only
	 * be effective when the image is saved as a png or gif.
	 * 
	 * <p>
	...
	/**
	 * Sets the size of each individual cell that constitutes a value in x,y,z data space. By setting the cell size, any previously set chart size will be overwritten with a value
	 * calculated based upon this value and the number of cells along each axis.
	 * 
	 * @param cellSize
	...
	/**
	 * Sets the width of the margin in pixels to be left as empty space around the heat map element. If a title is set then half the margin will be directly above the title and
	 * half directly below it. Where axis labels are set then the axis labels may sit partially in the margin.
	 * 
	 * <p>
	...
	public void setChartSize(Dimension newDim) {
		...
		chartSize = newDim;
		int heatMapWidth = newDim.width - axisThickness - yValuesHorizontalSize - yAxisLabelSize.height - (2 * margin), heatMapHeight = newDim.height
				- axisThickness - titleSize.height - xValuesVerticalSize - xAxisLabelSize.height - (2 * margin);
		cellSizeWidth = 1.0 * heatMapWidth / zValues[0].length;
		cellSizeHeight = 1.0 * heatMapHeight / zValues.length;
	}

	/**
	 * Sets the scale that is currently in use to map z-value to colour. A value of 1.0 will give a <strong>linear</strong> scale, which will spread the distribution of colours
	 * evenly amoungst the full range of represented z-values. A value of greater than 1.0 will give an <strong>exponential</strong> scale that will produce greater emphasis for
	 * the separation between higher values and a value between 0.0 and 1.0 will provide a <strong>logarithmic</strong> scale, with greater separation of low values. Values of 0.0
	 * or less are illegal.
	 * 
	 * <p>
	...
	/**
	 * Sets the String that will be used as the title of any successive calls to generate a chart. The title will be displayed centralised horizontally at the top of any generated
	 * charts.
	 * 
	 * <p>
	...
	/**
	 * Sets the String that will be displayed as a description of the x-axis in any generated charts. The label will be displayed horizontally central of the x-axis bar.
	 * 
	 * <p>
	...
	/**
	 * Sets the frequency of the values displayed along the x-axis. The frequency is how many columns in the x-dimension have their value displayed. A frequency of 2 would mean
	 * every other column has a value and a frequency of 3 would mean every third column would be given a value.
	 * 
	 * <p>
	...
	/**
	 * Sets the x-values which are plotted along the x-axis. The x-values are calculated based upon the indexes of the z-values array:
	 * 
	 * <blockcode>
	 * 
	 * <pre>
	...
	/**
	 * Sets the x-values which are plotted along the x-axis. The given x-values array must be the same length as the z-values array has columns. Each of the x-values elements will
	 * be displayed according to their toString representation.
	 * 
	 * @param xValues
	...
	/**
	 * Sets the String that will be displayed as a description of the y-axis in any generated charts. The label will be displayed horizontally central of the y-axis bar.
	 * 
	 * <p>
	...
	/**
	 * Sets the frequency of the values displayed along the y-axis. The frequency is how many rows in the y-dimension have their value displayed. A frequency of 2 would mean every
	 * other row has a value and a frequency of 3 would mean every third row would be given a value.
	 * 
	 * <p>
	...
	/**
	 * Sets the y-values which are plotted along the y-axis. The given y-values array must be the same length as the z-values array has columns. Each of the y-values elements will
	 * be displayed according to their toString representation.
	 * 
	 * @param yValues
	...
	/**
	 * Replaces the z-values array. See the {@link #setZValues(double[][], double, double)} method for an example of z-values. The smallest and largest values in the array are used
	...
	/**
	 * Replaces the z-values array. The number of elements should match the number of y-values, with each element containing a double array with an equal number of elements that
	 * matches the number of x-values. Use this method where the minimum and maximum values possible are not contained within the dataset.
	 * 
	 * <h2>Example</h2>
	 * 
	 * <blockcode>
	...
	/*
	 * Calculate and update the field for the distance between the low colour and high colour. The distance is the number of steps between one colour and the other using an RGB
	 * coding with 0-255 values for each of red, green and blue. So the maximum colour distance is 255 + 255 + 255.
	 */
