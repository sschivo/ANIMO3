...
public class CsvWriter {
	...
	public void writeCsv(File file, Model m, LevelResult r) throws IOException {
		...
		/*
		 * Object[] zioLupoMannaro = levels.keySet().toArray(); String[] reactantNames = new String[zioLupoMannaro.length]; for (int i = 0; i < zioLupoMannaro.length; i++) {
		 * reactantNames[i] = zioLupoMannaro[i].toString(); } bw.write("Time, "); for (int i = 0; i < reactantNames.length - 1; i++) { bw.write(reactantNames[i] + ", "); }
		 * bw.write(reactantNames[reactantNames.length - 1]); bw.newLine();
		 */
		...
		/*
		 * int curTime = levels.get(reactantNames[0]).firstKey(); while (true) { bw.write(curTime + ", "); for (int i = 0; i < reactantNames.length - 1; i++) {
		 * bw.write(levels.get(reactantNames[i]).get(curTime) + ", "); } bw.write("" + levels.get(reactantNames[reactantNames.length - 1]).get(curTime)); bw.newLine(); int nextTime
		 * = curTime + 1; levels.get(reactantNames[0]).tailMap(nextTime); if (levels.get(reactantNames[0]).tailMap(nextTime).isEmpty()) { break; } else { curTime =
		 * levels.get(reactantNames[0]).tailMap(nextTime).firstKey(); } } bw.close();
		 */
	 ...
	 * 
	 * @param filename
	 *            the filename of the file to output to
	 * @param m
	 *            the model to use as a base
	 * @param r
	 *            the results to save
