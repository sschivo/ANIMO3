...
public class ParameterFitter extends WindowAdapter
{
    ...
    // Please notice that currentSettingIndex counts the computation index as we start it (so that we can keep the results ordered), while nComputations counts the number of computations we have finished
    ...
    private void addReactionVariableParameters(JPanel panel, JCheckBox check, final Reaction reaction)
    {
        ...
        for (final String parName : parameters.keySet())
        {
            ...
            final JRadioButton linearScale = new JRadioButton("Linear"), logarithmicScale = new JRadioButton("Logarithmic");
            ...
            minValue.getDocument().addDocumentListener(new MinValueDocumentListener(reaction, parName, maxValue, minValue, incrementValue, logarithmicScale));
            ...
            maxValue.getDocument().addDocumentListener(new DocumentListener()
            {
                ...
                {
                    setVariableParameter(reaction, parName, new Double(minValue.getValue().toString()), new Double(maxValue.getValue().toString()), new Double(
                            incrementValue.getValue().toString()), logarithmicScale.isSelected());
                    updateTotalComputations();
                }
    ...
    private boolean compareResults(File myCSV) throws IOException
    {
        for (Reactant reactant : reactantComparisons.keySet())
        {
            ...
            BufferedReader myIS = new BufferedReader(new FileReader(myCSV)), hisIS = new BufferedReader(new FileReader(hisCSV));
            ...
            StringTokenizer myTritatutto = new StringTokenizer(myFirstLine, ","), hisTritatutto = new StringTokenizer(hisFirstLine, ",");
            ...
            while (hisLine != null)
            {
                ...
                while (myLine != null)
                {
                    ...
                    { //TODO: interessante notare che la differenza calcolata cos� mi esclude paradossalmente i grafici pi� belli... o_O
                        myIS.close();
                        hisIS.close();
                        return false;
                    }
    ...
    private Pair<Boolean, Double> compareResults(LevelResult myResult) throws IOException
    {
        ...
        for (Reactant reactant : reactantComparisons.keySet())
        {
            ...
            double hisNLevels = 15, myNLevels = model.getProperties().get(Model.Properties.NUMBER_OF_LEVELS).as(Integer.class);
    ...
    public JComponent createReactionComparisonPanel(final Model model)
    {
        ...
        csvFileNameTextField.getDocument().addDocumentListener(new CSVDocumentListener(comparisonBox, csvFileNameTextField, comparisonPanel));
        comparisonBox.add(Box.createGlue());
    ...
    {
        {
            for (Reaction reaction : scenarioFittingParameters.keySet())
            {
                ...
                for (String parName : reaction.getScenarioCfg().getParameters().keySet())
                {
                    if (scnFitting.getParameterSetting(parName).isFixed()
                            && !scnFitting.getParameterSetting(parName).getFixedValue().equals(reaction.getScenarioCfg().getParameters().get(parName)))
                    {
            ...
            updateTotalComputations();
            nComputations = 0;
            // Please notice that nSettings counts the computation index as we start it (so that we can keep the results ordered), while nComputations counts the number of computations we have finished
    ...
    public void showWindow(boolean exitOnClose, int nMinutesToSimulate)
    {
        ...
        window.setBounds((int) (Animo.getCytoscape().getJFrame().getWidth() * 0.2), (int) (Animo.getCytoscape().getJFrame().getHeight() * 0.2), (int) (Animo
                .getCytoscape().getJFrame().getWidth() * 0.6), (int) (Animo.getCytoscape().getJFrame().getHeight() * 0.6));
        if (exitOnClose)
        {
    ...
    public void visitParameterSettings(List<Pair<ScenarioCfg, ParameterSetting>> parSettings, int startIndex, ThreadPool pool)
    {
        if (terminate)
        {
            return;
            ...
            for (final AcceptableConfiguration acceptableConfiguration : acceptableConfigurations)
            {
                ...
                try
                {
                    ...
                    for (Reactant r : model.getReactantCollection())
                    {
                        ...
                        {
                            // I comment this so I show all and only the reactants selected for comparison. If you remove this comment, you can also get those reactants marked as plotted, and they will be hidden if they are not in a comparison || r.get(Model.Properties.PLOTTED).as(Boolean.class)) {
                        ...
                        for (String parName : scnFitting.getParameterSettings().keySet())
                        {
                            ...
                            if (!parSetting.isFixed())
                            {
                                builder.append(parName + "="
                                        + decimalFormat.format(acceptableConfiguration.getScenarioConfigurations().get(r.getId()).getParameters().get(parName))
                                        + ",");
                            }
                        }
                        builder.append("]; ");
                        ...
                        public void actionPerformed(ActionEvent e)
                        {
                            ...
                            for (Reaction r : scenarioFittingParameters.keySet())
                            {
                                ...
                                for (String parName : scnFitting.getParameterSettings().keySet())
                                {
                                    ...
                                    {
                                        CyEdge edge = network.getEdge(Long.parseLong(r.getId()));
                                        network.getRow(edge).set(parName,
                                                acceptableConfiguration.getScenarioConfigurations().get(r.getId()).getParameters().get(parName));
                                    }
                                }
                            }
    ...
    class MinValueDocumentListener implements DocumentListener
    {
        ...
        public MinValueDocumentListener(Reaction reaction, String parName, JFormattedTextField maxValue, JFormattedTextField minValue,
                JFormattedTextField incrementValue, JRadioButton logarithmicScale)
        {
            this.reaction = reaction;
            this.parName = parName;
    ...
    class TaskThread extends Thread
    {
        ...
        public TaskThread(Model analyzedModel, HashMap<String, ScenarioCfg> currentConfiguration, int actualIndexToRemember)
        {
        ...
        public void run()
        {
            try
            {
                ...
                allConfigurations.set(actualIndexToRemember,
                        new AcceptableConfiguration(currentConfiguration, result, "Max abs diff: " + decimalFormat.format(comparisonResult.second)));
            }
